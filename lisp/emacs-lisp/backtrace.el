  ...
  (let ((frames nil)
    ...
    (dotimes (idx (length frames))
      (let ((frame (nth idx frames)))
        ;; TODO: Is backtrace--locals supposed to work when idx is 0?
        (when (> idx 0)
...
;;   "Function for inserting a backtrace frame at point.
;; It is called with two arguments, FRAME and VIEW.  FRAME is a
;; backtrace-frame object, and VIEW is a plist as described in
;; `backtrace-view'.")
...
;; TODO copied this from tabulated-list-mode, is it really necessary?
...
POS, if omitted or nil, defaults to point."
  (let ((frame-end (backtrace-get-frame-end pos))
        (section-end (next-single-property-change
                         (or pos (point)) 'backtrace-section)))
...
(defun backtrace-describe-point ()
  ;; TODO remove
  (interactive)
  (message "Frame: %s section: %s form: %s view: %s"
           (backtrace-get-index)
...
For the cross-reference format, see `help-make-xrefs'."
  (interactive "d")
  ;; TODO
  ;; The following description was written before the fix to bug#8147.
  ;; (This function used to be called debug-help-follow in debug.el.):
  ;;
  ;;   Ideally we'd just do (call-interactively 'help-follow) except
  ;;   that this assumes we're already in a *Help* buffer and reuses
  ;;   it, so it ends up incorrectly "reusing" the *Backtrace* buffer
  ;;   to show the help info.
  ;;
  ;; The fix to #8147 signals an error instead of reusing the buffer.
  ;; So the code below which used to work around the bug described
  ;; above now works around the error.  There's a second patch
  ;; suggested in the email thread for that bug which was never
  ;; applied but which looks like it would make this workaround
  ;; unnecessary.
  (unless pos
    (setq pos (point)))
