...
;; will work if not byte-compiled, but it will not be able
;; to clean up backtraces correctly unless it is byte-compiled.
	   ...
	   (setq skip-next-lambda t
          ...
          ((pred edebug--symbol-not-prefixed-p)
           ...
           (setq before-index nil
                 after-index nil))
          (`(,(or 'lambda 'closure) . ,_)
           (message "skip-next-lambda is %s at %s" skip-next-lambda index)
